  ...
  return (
    <div className="glass-card rounded-2xl p-5 space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <p className="text-sm font-bold" style={{ color: 'var(--c-1)' }}>Core pronunciation model</p>
          <p className="text-xs mt-1" style={{ color: 'var(--c-3)' }}>
            {technical ? 'Technical view' : 'Beginner view'} — switch depending on how much detail you want.
          </p>
        </div>
        <div className="flex items-center gap-1 rounded-lg p-0.5" style={{ background: 'var(--c-surface-2)', border: '1px solid var(--c-border)' }}>
          {[
            { key: 'beginner', label: 'Beginner', active: !technical },
            { key: 'technical', label: 'Technical', active: technical },
          ].map((option) => (
            <button
              key={option.key}
